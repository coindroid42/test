...
// Copyright (c) 2019 The DogCoin Core developers
...
Value stop(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "DogCoin server stopping";
}
...
void StartRPCThreads()
{
         ...
         (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) && Params().RequireRPCPassword())
    {
        ...
        uiInterface.ThreadSafeMessageBox(strprintf(
            _("To use dogcoind, or the -server option to dogcoin-qt, you must set an rpcpassword in the configuration file:\n"
              "%s\n"
              ...
              "for example: alertnotify=echo %%s | mail -s \"DOGC Alert\" admin@foo.com\n"),
...
std::string HelpExampleCli(string methodname, string args){
    return "> dogcoin-cli " + methodname + " " + args + "\n";
}
